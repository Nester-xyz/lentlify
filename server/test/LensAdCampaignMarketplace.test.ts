...
describe("LensAdCampaignMarketplace", function () {
  ...
  beforeEach(async function () {
    ...
    owner = Wallet.fromMnemonic(testMnemonic, "m/44'/60'/0'/0/0").connect(
      provider
    );
    seller = Wallet.fromMnemonic(testMnemonic, "m/44'/60'/0'/0/1").connect(
      provider
    );
    feeCollector = Wallet.fromMnemonic(
      testMnemonic,
      "m/44'/60'/0'/0/2"
    ).connect(provider);
    ...
    try {
      ...
      paymentToken = await deployer.deploy(mockERC20Artifact, [
        "GRASS Token",
        "GRASS",
      ...
      console.log(
        `MockGraphContract deployed at: ${await mockGraphContract.getAddress()}`
      );

      // Deploy MockContract for action hub
      const mockActionHub = await deployer.deploy(mockContractArtifact, []);
      console.log(
        `MockActionHub deployed at: ${await mockActionHub.getAddress()}`
      );

      // Deploy LensAdCampaignMarketplace
      const marketplaceArtifact = await deployer.loadArtifact(
      ...
      console.log(
        `LensAdCampaignMarketplace deployed at: ${await marketplace.getAddress()}`
      );
    } catch (error) {
  ...
  describe("Campaign Creation and Updates", function () {
    ...
    it("Should update campaign slots", async function () {
      ...
      try {
        ...
        const updateTx = await marketplace.updateCampaignSlots(
          campaignId,
          additionalSlots
        );
        ...
        try {
          ...
          expect(Number(updatedCampaign.availableSlots)).to.be.at.least(
            AVAILABLE_SLOTS
          );
        } catch (error: any) {
    ...
    it("Should update campaign content", async function () {
      ...
      try {
        ...
        const updateTx = await marketplace.updateCampaignContent(
          campaignId,
          newContentURI,
          newContentHash
        );
        ...
        try {
          const updatedCampaign = await marketplace.campaigns(campaignId);
          console.log(
            `Updated content URI: ${updatedCampaign.groveContentURI}`
          );
    ...
    it("Should update campaign price", async function () {
      ...
      try {
        ...
        const updateTx = await marketplace.updateCampaignPrice(
          campaignId,
          newRewardPrice
        );
        ...
        try {
          ...
          expect(updatedCampaign.rewardAmount.toString()).to.equal(
            newRewardPrice
          );
        } catch (error: any) {
  ...
  describe("Influencer Actions", function () {
    it("Should execute influencer action", async function () {
      ...
      try {
        ...
        const actionTypeKey = hre.ethers.keccak256(
          hre.ethers.toUtf8Bytes("lens.param.actionType")
        );
        const actionTypeValue = hre.ethers.AbiCoder.defaultAbiCoder().encode(
          ["uint8"],
          [ActionType.MIRROR]
        );
        ...
        try {
          ...
          try {
            const hasParticipated = await marketplace.hasParticipated(
              0,
              owner.address
            );
