...
const CampaignPost: React.FC = () => {
  ...
  const fetchCampaignParticipants = async (campaignId: number) => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `Found ${allParticipants.length} participants for campaign ${campaignId} using direct getter`
        );
      } catch (e) {
        // Fall back to the old method if the new function isn't available
        console.log(
          "Direct participant getter not available, falling back to old method"
        );
        allParticipants = await getCampaignParticipants(campaignId);
        console.log(
          `Found ${allParticipants.length} participants for campaign ${campaignId} using fallback method`
        );
      }
  ...
  const fetchParticipantActions = async (
    campaignId: number,
    participantAddresses: `0x${string}`[]
  ) => {
    ...
    try {
      ...
      for (const address of participantAddresses) {
        const influencerActions = await getCampaignInfluencerActions(
          campaignId,
          address
        );
        ...
        if (influencerActions && Array.isArray(influencerActions)) {
          ...
          for (const action of influencerActions) {
            ...
            try {
              profileData = await fetchLensProfileByAddress(
                action.influencerAddress
              );
            } catch (error) {
  ...
  useEffect(() => {
    const fetchCampaign = async () => {
      ...
      try {
        setIsLoading(true);
        console.log(
          `Fetching campaign ${pId} from contract ${CONTRACT_ADDRESS}`
        );
  ...
  const handleExecuteAction = async (campaign: CampaignData) => {
    ...
    try {
      ...
      console.log(
        `Executing Lens action type: ${actionType} on post ${campaign.postId}`
      );
      console.log("Campaign details:", campaign);

      // Debug auth status at the time of action execution
      console.log("Auth status at execution time:", {
        isAuthorized,
        sessionClient,
        profile,
      });
      ...
      if (!sessionClient || !isAuthorized) {
        console.error(
          "No Lens session client available or not authorized. Please make sure you're logged in to Lens Protocol."
        );
      ...
      switch (actionType) {
        case 1: // MIRROR (Repost)
          ...
          if (result && !("isErr" in result && result.isErr())) {
            ...
            console.log(
              "Recording action in contract using Lens Protocol smart wallet"
            );
            ...
            try {
              ...
              console.log(
                "Using Lens Protocol smart wallet for contract interaction"
              );
              ...
              console.log(
                "Lens Protocol smart wallet transaction result:",
                contractResult
              );
              ...
              console.error(
                "Error with Lens Protocol smart wallet transaction:",
                contractError
              );
        ...
        case 2: // COMMENT
          ...
          const commentText =
            campaign.metadata?.commentText || "Great content! #lentlify";
          ...
          const commentUpload = await storageClient.uploadAsJson(
            commentMetadata
          );
          ...
          if (result && !("isErr" in result && result.isErr())) {
            ...
            try {
              ...
              console.log(
                "But the comment post was still created successfully"
              );
            }
          } else {
        ...
        case 3: // QUOTE
          ...
          const quoteText =
            ...
            "Check out this interesting content! #lentlify";
          ...
          try {
            console.log(
              "Creating quote with Lens Protocol smart wallet integration"
            );
            ...
            if (result && !("isErr" in result && result.isErr())) {
              ...
              console.log(
                "Step 2: Recording action in contract using Lens Protocol smart wallet"
              );
              ...
              try {
                ...
                console.log(
                  "But the quote post was still created successfully"
                );
              }
            } else {
  ...
  return (
    <Page
      pageHeading={campaign?.metadata?.title || "Campaign Post"}
      title={campaign?.metadata?.title || "Campaign Post"}
    >
            ...
            <div className="p-6">
                ...
                <div className="bg-gray-700 p-4 rounded-md">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center text-sm text-gray-300">
                      ...
                      <span>
                        Action:{" "}
                        {getActionTypeName(campaign.metadata?.actionType || 0)}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center text-sm text-gray-300">
                      ...
                      <span>
                        Min Followers:{" "}
                        {campaign.minFollowersRequired?.toString() || "0"}
                      </span>
                    </div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-2">
                          {campaign.metadata.categories.map(
                            (category: string, index: number) => (
                              <span
                                key={index}
                                className="bg-gray-600 text-gray-300 px-2 py-1 rounded-full text-xs flex items-center"
                              >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center text-sm text-gray-300">
                      <span>
                        Reward:{" "}
                        {campaign.likeReward
                          ? (Number(campaign.likeReward) / 1e18).toFixed(4)
                          : "0"}{" "}
                        GHO
                      </span>
                    </div>
                    ...
                    <div className="flex items-center text-sm text-gray-300">
                      <span>
                        ...
                        {campaign.depositsToPayInfluencers
                          ? (
                              Number(campaign.depositsToPayInfluencers) / 1e18
                            ).toFixed(4)
                          : "0"}{" "}
                        GHO
                      </span>
              ...
              {campaign.status === 1 && (
                <div className="mb-6 p-4 bg-gray-700 rounded-md">
                  ...
                  {campaign.metadata?.actionType === ActionType.COMMENT &&
                  ...
                  {campaign.metadata?.actionType === ActionType.QUOTE &&
                    campaign.metadata?.quoteText && (
                      <div className="mb-2">
                        <div className="text-sm font-semibold text-gray-300 mb-1">
                          What to quote:
                  ...
                  {campaign.status === 1 && (
                    <button
                      className={`w-full mt-2 ${
                        campaign.endTime &&
                        Number(campaign.endTime) * 1000 < Date.now()
                          ...
                          : "bg-gray-600 cursor-not-allowed"
                      } text-white font-medium py-2 px-4 rounded-md flex items-center justify-center transition-colors`}
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                    ...
                    >
                      ...
                      {isContractLoading
                        ...
                        ? `${
                            ...
                            campaign.likeReward &&
                            BigInt(campaign.likeReward) > 0
                              ? (
                                  Number(BigInt(campaign.likeReward)) / 1e18
                                ).toFixed(4) + " GHO"
                              : "Completed"
                          }`
                        : "Campaign Full"}
