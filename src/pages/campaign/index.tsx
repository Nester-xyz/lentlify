...
import { fetchLensProfileByAddress } from "@/lib/lens";
...
const CreateCampaignGroup: React.FC = () => {
  ...
  const { campaignGroups, isLoading, error, fetchCampaignGroups } =
    useCampaignContext();

  const [ownerProfiles, setOwnerProfiles] = useState<
    Record<string, LensProfile | null>
  >({});
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      if (campaignGroups.length > 0) {
        ...
        const newProfiles: Record<string, LensProfile | null> = {};
    ...
    <div className="w-full min-h-screen  dark:bg-gray-900 dark:text-white">
      ...
      >
        {campaignGroups.map((group: CampaignGroupData) => (
          ...
          >
            {/* Cover Photo Section */}
            <div className="relative h-28 md:h-32 bg-gradient-to-r from-gray-700 to-gray-900 dark:from-gray-600 dark:to-gray-800">
